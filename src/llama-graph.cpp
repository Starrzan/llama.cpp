...
ggml_tensor * llm_graph_context::build_attn(
    ...
    // TurboQuant: dequantize KV to F32 (still in WHT domain), then apply
    // forward WHT to Q so all tensors are in the same domain for attention.
    // After attention, inverse WHT on output to return to original domain.
    {
        auto is_turbo = [](ggml_type t) -> bool {
            return t == GGML_TYPE_TURBO2_0 || t == GGML_TYPE_TURBO3_0 ||
                   t == GGML_TYPE_TURBO4_0;
        };
        bool use_turbo = is_turbo(k->type) || is_turbo(v->type);
        ...
        if (use_turbo) {
            // Forward WHT on Q to match K/V turbo domain
            if (!ggml_is_contiguous(q)) { q = ggml_cont(ctx0, q); }
            ggml_tensor * scale = mctx_cur->get_turbo_innerq_scale_inv();
            q = ggml_turbo_wht(ctx0, q, 0, 0, scale);
        }
    }

    ggml_tensor * cur = build_attn_mha(q, k, v, kq_b, kq_mask, sinks, v_mla, kq_scale, il);

    // Inverse WHT on attention output to return to original domain
    {
        auto is_turbo = [](ggml_type t) -> bool {
            return t == GGML_TYPE_TURBO2_0 || t == GGML_TYPE_TURBO3_0 ||
                   t == GGML_TYPE_TURBO4_0;
